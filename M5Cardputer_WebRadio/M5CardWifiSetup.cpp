...
M5CardWifiSetup::M5CardWifiSetup() {
}

String M5CardWifiSetup::inputText(const String& prompt, int x, int y) {
    String data = "> ";
...
void M5CardWifiSetup::displayWiFiInfo() {
...
void M5CardWifiSetup::connectToWiFi() {
    ...
    delay(200);
    CFG_WIFI_SSID = preferences.getString("wifi_ssid", "");
    CFG_WIFI_PASS = preferences.getString("wifi_pass", "");
    preferences.end();
    WiFi.disconnect();
    ...
    if (WiFi.status() == WL_CONNECTED) {
        ...
        Preferences preferences;
        preferences.begin("wifi_settings", false);
        preferences.putString("wifi_ssid", CFG_WIFI_SSID);
        preferences.putString("wifi_pass", CFG_WIFI_PASS);
        preferences.end();
        M5Cardputer.Display.clear();
...
String M5CardWifiSetup::scanAndDisplayNetworks() {
