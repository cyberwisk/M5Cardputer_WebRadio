...
String scanAndDisplayNetworks() {
    ...
    if (numNetworks == 0) {
        ...
        M5Cardputer.Display.clear();
        M5Cardputer.Display.drawString("Selecione o SSID:", 1, 1);
        int selectedNetwork = 0; 
        while (1) {
            for (int i = 0; i < 6 && i < numNetworks; ++i) {
            ...
            M5Cardputer.Display.drawString("Selecione o SSID:", 1, 15 + numNetworks * 15);
            M5Cardputer.update();
            if (M5Cardputer.Keyboard.isChange()) {
                if (M5Cardputer.Keyboard.isPressed()) {
                    Keyboard_Class::KeysState status = M5Cardputer.Keyboard.keysState();
                    ...
                    if (M5Cardputer.Keyboard.isKeyPressed('.') && selectedNetwork < min(5, numNetworks - 1)) {
                        selectedNetwork++;
                    }
                    if (status.enter) {
